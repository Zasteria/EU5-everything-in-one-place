...
# Куплено во второй раз 2026-09-22: `nmt_ru` собрался, прошёл все проверки,
# был скопирован в игру руками — и **лаунчер его не показал**. Единственным
# отличием от двадцати трёх остальных `metadata.json` в дереве оказалось
# отсутствие ключа `relationships`. Поэтому набор здесь — не «то, без чего
# точно не работает», а **весь набор, который везут все работающие моды**:
# лаунчер молчит о причине, и дешевле повторить чужой файл целиком.
