...
**Раздачу грамот не правят на глаз** — две правки 09-14 ушли впустую:
`tools/rights_sim.py <дамп>` прогоняет её на его числах и повторяет реальный
заход, `--ceilings` даёт потолок каждой грамоты. **Раздача одна на оба режима**:
специализация звала свою, без лестницы, и оставляла по три грамоты с нулём.
Отставание — **уклон** (`_rlagw<k>`), считается в **городах**, проходит любую
полосу; галочка «Грамоты — поровну» (по умолчанию) решает, брать ли отставшей
землю, которая ей не платит. **Грамота ставится руками** и **«+1»/«−1» ходят
провинцией**. Разбор —
