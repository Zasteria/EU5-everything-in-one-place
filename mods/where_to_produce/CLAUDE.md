...
**`_cov_pass` -- вызов `_trmm_*`, а не пересказ** (09-14): пересказ терял
вычитание выданной грамоты. **Отличие от карты одно: лучший способ из
доступных.** **У каждого окна свой тумблер эпохи**, и проход читает тумблер
позвавшего (`_cov_age_plan` / `_cov_age_rank` → `_cov_end`); подсказки эпоху
называют (`_age_*`). **Разбор в нашей подсказке — наш**, чужой ловит
`foreign_breakdown_in_own_text`. **Прогона не видело.**
