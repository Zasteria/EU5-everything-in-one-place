...
**Окно замены принято прогоном 09-14** (правый список и процент в нём); «−» и
`_cov_pass` чинены и **прогона не видели** —
[`wtp_backlog.md`](../../docs/investigations/wtp_backlog.md).

**Подсказка «Пригодности» — наш счёт, и до 09-14 была чужим** (столбец `_rq<k>`,
подсказка — разбор карты CM: 236 % против 76.1 % на одном экране). Теперь в ней
лесенка прав `_rqr<k>` и разбор по товарам: итог = сырьё + модификатор, порознь,
со способом и чипами сырья. **Порядок строк не тронут — причину назвать нечем,
пока он не посмотрит разбор.**
