...
`tools/rights_sim.py <дамп>` гоняет её на его числах, `--ceilings` даёт потолок
каждой грамоты. **У специализации раздача своя, и лестницы в ней нет нарочно**:
«лучшее ставить, и в ней вполне может быть 0 каких-то прав» — ровности спрашивать
у обычного плана. Там отставание — **уклон** (`_rlagw<k>`), в **городах**,
проходит любую полосу; галочка «Грамоты — поровну» (снята) решает, брать ли
отставшей землю, которая ей не платит. **Грамота ставится руками**, **«+1»/«−1»
ходят провинцией**. Разбор —
