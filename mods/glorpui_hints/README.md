...
## Форк: что взято у Glorp UI

Владелец снял Glorp UI с плейсета (2026-09-14). По числам мод дорогим не был —
[`../../docs/investigations/glorp_ui_cost.md`](../../docs/investigations/glorp_ui_cost.md)
— но нужны от него были только подсказки и полоски в окне ценностей, и его
решение: «просто спизди нужные вещи из glorp. Ничего потом сверяться не нужно,
любые косяки мы самостоятельно изменим и любые обновления уже свои сделаем».

**Взято четыре их генерируемых файла**, `tools/fork_hints.py`:

| их файл | здесь | что в нём |
| --- | --- | --- |
| `glorpui_generated_societal_value_hint_loc.txt` | `svx_svh_hint_loc.txt` | по одной `customizable_localization` на строку подсказки: триггер доступности и ключ, который печатать |
| `glorpui_generated_societal_value_hint_script_values.txt` | `svx_svh_hint_script_values.txt` | по одному script value на конец оси: ненулевое, пока на этом конце есть хоть одна доступная строка |
| `glorpui_societal_value_hint_scripted_triggers.txt` | `svx_svh_triggers.txt` | три гейта доступности: реформа, политика, привилегия |
| `glorpui_societal_value_hint_warning_suppression.txt` | `svx_svh_suppression.txt` | объявляет две сцены, которые ставит `.gui` |

**Переименовано, а не скопировано, и это не эстетика.**
`customizable_localization` переопределить нельзя — выигрывает первое
прочитанное определение, остальные выбрасываются с `gamedatabase.h: Duplicated
key`. Под их именами в плейсете с обоими модами было бы нельзя сказать, чьё
определение работает. Под `svx_svh_*` коллизии нет вовсе. Переименование идёт
одним проходом `fork_hints.rename`, применённым **к каждому файлу, который
пишет любой генератор** — потому что имена сцен ставит `.gui`, а читают их
script values, и разъехаться они не должны.

**Одно убрано, а не переименовано.** В их script values стоит
`NOT = { has_variable = showUnavailableSocietalValueSuggestions }` — их
собственный переключатель, который здесь некому ставить. Клауза вырезается (34
штуки, по одной на конец оси, число печатается сборкой), а то же самое решает
теперь `svx__show_all` в `.gui`: **выключен — отфильтрованные списки, включён —
весь ванильный блоб**. Ровно их логика, на нашей настройке.

**Все одиннадцать языков шлются отсюда.** До форка Glorp UI сам вёз текст, и
`SHIP_GLORP_HINTS` держал здесь только русский; теперь везти его больше некому.
Слова по-прежнему стоят полсотни строк на язык — зачины в `languages.py`, —
потому что каждая подсказка это зачин, ссылка `$key$` и число.

**Что проверяется сборкой.** `check_references_resolve` доказывает, что каждый
`ScriptValue`, каждый `Player.Custom` и каждый `localization_key` разрешается
внутри этого мода: 1 072 определения, ноль дублей, 982 вызова, 983 ключа. От
Glorp UI в собранном моде не остаётся ни одного имени — только упоминания в
комментариях о происхождении.

**И `.metadata`:** зависимость `glorp.ui` снята. В названии мода их имя пока
осталось — вещь опубликована, и переименование решает владелец.

### Что проверить

Форк **в игре не был**. Один заход без Glorp UI:

1. Подсказка любой ценности — должен вернуться главный список «Чтобы сместить
   дальше влево/вправо», отфильтрованный по тому, что держава может взять.
2. Мод-меню CMF → «Показывать всё без фильтра» **включить**: главный список
   должен смениться на ванильный, длинный и нефильтрованный.
3. `error.log`: ни `Duplicated key`, ни жалоб на `svx_svh_*`.
