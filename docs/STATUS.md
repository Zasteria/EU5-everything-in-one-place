...
| [`cm_perf`](../mods/cm_perf/CLAUDE.md) | **потолок скорости снят 09-18, но 09-19 он снял мод: тормозит тем сильнее, чем больше точек авторасширения.** Копия CM. Месячный пульс трогать нельзя — его слово 09-19. Ставится **вместо** CM | правка 3 |
| [`widget_probe`](../mods/widget_probe/CLAUDE.md) | **отладочный. Прогон 09-19: `PdxGuiDestroyWidget` из мода работает**, вверху дерева утечки нет. Свёрнут в кнопку, переписывает корень через `AccessChild` и считает детей у окон CM | перепись корня, счётчики CM, `gui.clearwidgets` |
