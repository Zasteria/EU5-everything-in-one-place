...
| [`widget_probe`](../mods/widget_probe/CLAUDE.md) | **отладочный, в игре не запускался.** Окно-пробник: цепочка родителей с числом детей, жертва под `PdxGuiDestroyWidget`, кнопки на `gui.clearwidgets` и `CloseAllTooltips`. Ставится на один прогон и снимается | всё: ни один из трёх вызовов не проверен |
