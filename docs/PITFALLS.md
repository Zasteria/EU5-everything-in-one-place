...
этого мода, где та же форма работает для `BuildingType`. **Прежде чем называть
что-то невозможным: (1) все пять дампов, не один; (2) чем скрипт отдаёт объекты
интерфейсу; (3) что уже умеет своё же окно.** Владелец не поверил и был прав —
круг оплачен его временем.
