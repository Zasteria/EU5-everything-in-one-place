...
**Мода без `game_custom_data` в `metadata.json` лаунчер не показывает вовсе**, и
не говорит почему. 2026-09-20: `marker_throttle` собрался, прошёл все проверки и
не появился в списке модов. Одиннадцать работающих модов здесь везут этот ключ,
а новый — нет. Теперь ключи сверяет `tools/check_script.py`.
