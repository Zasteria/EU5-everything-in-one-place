...
- Значки — **приток**: каждый отряд, каждая стройка, каждый тултип создаёт
  виджеты. Сообщество право в этом.
- Убрать значки — значит остановить приток, а не убрать накопленное. Виджеты,
  которые час назад сделали значки, по-прежнему в процессе, просто не рисуются.
- А всё, что обходит дерево, платит за накопленное, а не за видимое: покадровые
  ворота и `PdxGuiTriggerAllAnimations` (глобальная, слив очереди CM зовёт её
  кругами).
...
**Два рычага, и они разные.** Урезать покадровый счёт ванильных значков
(переопределить типы) — это про приток и про FPS. Освободить накопленное —
единственное настоящее лечение, и упирается оно в то, что уничтожить можно только
названное по имени. Непроверенный рычаг ровно один: консольная команда
`gui.clearwidgets` (`ui_library.gui:18261`), кнопка на неё в зонде есть, **её ни
разу не нажимали**.
