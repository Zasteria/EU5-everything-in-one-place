...
## Счёт «что тут может стоять» обязан спрашивать локацию

**Покрытие спрашивало только державу, и потому предлагало городу сельское
здание, а Вестфалии японское.** «Стекло 100 % — Сельский стекольщик»,
«Каменная кладка — Сёэн, Ремесленник дайку» (его прогон 2026-09-14). Ворота
доступности державе (`can_build_building` в скоупе страны) отвечают на «умеет ли
страна», и ни слова — про ранг локации, её `location_potential` и тумблер
город/село. Это `_stands_<здание>`, и без него любой такой счёт — бред, который
выглядит числом.

**И «на конец игры» — это не «сейчас без ограничений».** Ветка, которой сняли
проверку доступности, пустила здания, замененные наследником: подсказка «на
конец» предлагала скрипторий, которого в конце игры не бывает. Лестница
`obsolete` — статический факт игры; ветка такого здания в режиме «на конец»
просто не должна открываться.

**А `_stands_<здание>` под галочкой ранга сам терял ранг — та же ошибка
этажом ниже.** Галочка «город»/«село» существует затем, чтобы переопределить
**только ранг**: где она стоит, `can_build_building` заменяется на
`location_potential` здания. Заменялось — а сам ранг не спрашивался ничем, и
здание, объявившее одну сторону, отвечало «да» на обеих. У `market_village`
`location_potential` нет вовсе, поэтому под галочкой «город» триггер был голым
`_rank_is_ticked = yes` — истина всегда. Его прогон 2026-09-14: ювелирная
грамота 100 % на городе, а в разборе «Торговая деревня, Сельский ювелир 1/1»,
«а деревни в целом не могут вставать в города».

**Сторона — `eu5data.Method.urban` / `.rural`, те же две, которыми делит
`plan_groups`**, чтобы триггер и раздача не могли разойтись: 77 зданий только
для города получили `_plan_is_town = yes`, 8 только для села — `= no`,
двусторонние остались без теста. **Правило общее: подменяя вопрос игры своим,
перечисли всё, что игра в нём спрашивала.** `can_build_building` — это ранг
**и** `location_potential`; замена, которая вернула одно из двух, тише, чем та,
которая не вернула ничего.
...
**Пересказ чужой формулы теряет ту строку, ради которой она и написана.** Счёт
земли под грамоту, пересказанный из Construction Manager, потерял вычитание уже
выданной грамоты и размазал частный случай `dyes`/`wine` на все двадцать
товаров. Обе потери, числа и его прогон 2026-09-14 —
[`../archive/pitfalls_script_transferred_formula.md`](../archive/pitfalls_script_transferred_formula.md).
**Если чужое значение уже лежит в дереве — зови его**, а не пиши рядом второе с
тем же именем в комментарии.
