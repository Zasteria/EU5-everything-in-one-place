...
**Дорогим он не был** ([`investigations/glorp_ui_cost.md`](investigations/glorp_ui_cost.md)),
но снят по его решению. **09-14 форкнуто** четыре их генерируемых файла под
именами `svx_svh_*` («просто спизди нужные вещи… любые обновления уже свои
сделаем»), одиннадцать языков шлются отсюда, зависимость `glorp.ui` снята.
**Имя мода — всё ещё их, это его решение.** **Прогона форк не видел**; что
смотреть — в [`../mods/glorpui_hints/README.md`](../mods/glorpui_hints/README.md#форк-что-взято-у-glorp-ui).
...
**Что нужно от него, одно сообщение:** (1) папка `logs` **упавшего запуска**,
