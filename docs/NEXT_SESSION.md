...
**Непроверенное в сборке:** «+» в окне замены (ставит здание, по которому нажал),
своё покрытие `bag_wtp_cov_pass` вместо числа карты, столбец «Пригодность» в
поиске по грамоте, ступень лестницы только верхняя, процент выгоды в строке.
Что именно смотреть — в конце
...
**Прежде чем чинить «молча не работает» —
[`pitfalls/how_to_fix.md`](pitfalls/how_to_fix.md).** Одно пустое окно стоило
пяти сборок, четыре из них были догадками «по правилу»; первый шаг — сравнить
устройство с тем, что в этом же моде рисуется.
