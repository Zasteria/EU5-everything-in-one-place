...
**Открыто: «Из плана — сюда» отстаёт на одну локацию.** Панель фильтрует список
тогда же, когда строит, а проба `bag_view_location` пишет локацию позже.
Пересобрать список из скрипта нечем, хука «локация выбрана» нет — **проба
переставлена в `panel_header`, и это именно проба.** Не поможет — рисовать список
плана текстом в самой панели: текст перечитывается каждый кадр. **Не прогонялось.**
