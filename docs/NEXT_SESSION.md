...
режим «провинцией» работают (строка нажатия называла район вместо провинции —
починено). **Компенсация отставшим не сработала, и причина названа:** воротами
была **полоса** (`_sprt >= _rband`), а не ключ — оружейным земля платит 6 %, то
есть 60 из 1000, и они не проходили ни одну полосу, кроме нулевой, к которой
земли уже не оставалось. Теперь отставшая проходит любую полосу, если земля
платит ей `> 0` и провинция у неё уже есть. **Эта правка прогона не видела**;
что смотреть — [`investigations/wtp_backlog.md`](investigations/wtp_backlog.md).
**Его потолок:** оружейным платят только две провинции этой земли — значит две
они и возьмут, не восемь.

**И `_stands_<здание>` под галочкой ранга теряло сам ранг** (09-14: ювелирная
грамота 100 % на городе, а в разборе «Торговая деревня»); теперь несёт сторону.
Заодно шапка столбца «Городское право» перестала звать подсказку, которой нужна
локация. **Прогона не видело.**
...
зданий, поделённое среднее провинции в лесенке. **Что смотреть — «Прогон: что
смотреть» в конце
