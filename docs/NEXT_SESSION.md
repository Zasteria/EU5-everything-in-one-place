...
**Окно замены принято целиком; «Пригодность» и специализация чинены дважды и
прогона не видели.** Обе причины назвал он, цифрами. Первая: `_cov_pass` был
**пересказом** `_trmm_cov_*`, а не его вызовом, и потерял вычитание уже выданной
здесь грамоты — отсюда «что за 160 % местного модификатора». Вторая: **у каждого
окна свой тумблер эпохи**, а проход читал тумблер окна плана всегда, так что
галочка в поиске не делала ничего; эпоха теперь приходит от вызывающего и
подписана в каждой подсказке, которая от неё зависит. **Что смотреть — «Прогон:
что смотреть» в конце
[`investigations/wtp_backlog.md`](investigations/wtp_backlog.md)**, пять пунктов.
