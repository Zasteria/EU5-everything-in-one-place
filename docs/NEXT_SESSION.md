...
**Окно замены и порядок «Пригодности» приняты прогоном 09-14** («Эйфель 79.4 %,
Эмсланд 76.1 %… всё сходится»). **Не видели прогона:** «на конец» как
`always = yes` там, где `_reach_` нет (раньше подставлялся `_avail_`, и конец
игры равнялся сегодняшнему дню); выброшенный модификатор вывода; лесенка без
недоступных державе и нулевых прав; три права в подробностях; специализация,
считающая лесенку тем же средним по городам провинции, каким выдаёт грамоту;
`ERROR` в имени способа. **Что смотреть — «Прогон: что смотреть» в конце
[`investigations/wtp_backlog.md`](investigations/wtp_backlog.md)**, шесть
пунктов.

**Логи он прислал впервые, и они дали три ошибки, невидимые на экране** —
`debug.log` важнее `error.log`: проход без нужного `scope:` (1 448 строк),
чтение отсутствующей переменной в строке окна (240) и `ai_weight` в воротах из-за
разбора `potential` (16). **Просить `debug.log` всегда.** Про вылеты в этих логах
нет ничего: ни дампа, ни исключения — сеанс 03:07–03:20 завершился нормально.
...
Живая работа не в `main`, а в `dev`. Взято: `.editorconfig` наполовину (без их
CRLF) и мысль `gui_update.py` как `stale_overrides`. Ждёт решения одно: их
`upload.py` со SteamworksPy — вечер работы и его Steam.
