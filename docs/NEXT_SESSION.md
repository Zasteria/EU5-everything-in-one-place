...
**Окно замены принято целиком, «Пригодность» починена и прогона не видела.**
Причину назвал он, цифрой: «что за 160 % местного модификатора вывода». Наш
`_cov_pass` был **пересказом** `_trmm_cov_*`, а не его вызовом, и потерял ту
строку, ради которой формула написана — вычитание уже выданной здесь грамоты;
плюс размазал усреднение своего РГО с двух товаров на двадцать. Теперь зовётся
его значение. **Что смотреть — «Четырнадцатый прогон» в конце
[`investigations/wtp_backlog.md`](investigations/wtp_backlog.md)**; главный
вопрос один: поменялись ли Эмсланд и Эйфель местами.
