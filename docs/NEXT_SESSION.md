...
**Почему CM ест производительность — измерено и записано**,
[`investigations/cm_performance.md`](investigations/cm_performance.md): одно
всегда живое окно с `visible`, который нарочно всегда истина, и тремя вложенными
датамоделями по всем типам зданий. **Мы вторые в той же таблице**, и спасает нас
единственное: `visible` на самом окне у всех восьми — это теперь стережёт
`ungated_live_windows`.

**Просить `debug.log`, а не только `error.log`:** три ошибки из четырёх за 09-14
нашлись только там. Про вылеты в присланных логах нет ничего — ни дампа, ни
исключения.
