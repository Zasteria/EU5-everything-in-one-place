...
| Does a building with two production slots earn one RGO bonus or two? | **Two, one per method, each over its own output** — the build panel's tooltip is headed with the *method's* name, so a pair is the summed output at the output-weighted blend. | TESTLOG 2026-08-31, twenty-first load |
