...
**2026-09-14, двадцатый заход — прогон был не в том режиме, и это видно в дампе.**
Сборка `5293f8`, первый дамп по грамотам. Его слово: «эффект стал прямо
противоположенным, пива стало ещё больше, пушек ещё меньше».

**Дамп отвечает первой же строкой:** «РЕЖИМ СПЕЦИАЛИЗАЦИИ: равномерности не
было», `WTP PASS … mode=1`. Правка отставания живёт в общей раздаче
(`_plan_grant_pass_prov`), а специализация раздавала грамоты **своим** проходом,
у которого не было ни лестницы, ни полос, ни квоты: каждая провинция брала одну
лучшую грамоту, и всё. Прошлый прогон был в обычном режиме, этот — в
специализации; сравнивались два разных прохода, а не «до» и «после».

| грамота | винокур. | ремесл. | текстил. | книгопеч. | ювелир. | каменные | корабел. | оружейн. | инструм. |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| его экран | **15** | 6 | 9 | 5 | 4 | **0** | **0** | **0** | **0** |

**Дамп же дал и потолки — то, чего до сих пор не знал никто.** Сколько городов
грамота вообще может взять на этой земле: винокуренные 36, ремесленные 26,
текстильные 21, каменные 20, книгопечатные 12, корабельные 12, **оружейные 8**,
ювелирные 4, **инструментальные 0** (им не платит ни одна из двенадцати
провинций). Значит его «оружейным платят только две провинции» было занижено
вдвое, а инструментальные не получат ничего никогда и никаким правилом.

**Прогон стоил двух правок, которые ничего не меняли, и из этого вырос
`mods/where_to_produce/tools/rights_sim.py`** — раздача, прогнанная на числах дампа. Он повторяет этот
заход город в город, и обе прошлые догадки видны в нём за секунду.

**Починено, прогона не видело.** Специализация раздаёт грамоты общей раздачей
(разброс 15 → 7, нулей 3 → 0), отставание считается уклоном, а не обрывом, и
галочка «Грамоты — поровну» (стоит по умолчанию) решает, брать ли отставшей
землю, которая ей не платит.
