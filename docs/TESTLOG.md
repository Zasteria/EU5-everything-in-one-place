...
**2026-09-20, `marker_throttle` 0.1.1 — тормоз ломает выбор отрядов.** Его
слова: «с самого начала игры стало ясно, что отряды стали не кликабельными. Я не
мог никого из них выбрать, хотя сами значки визуально были. Их можно выбрать
если выделить область на экране мышью, но не щелчком». Скорость тиков оценить не
успел.

**Что это говорит:** `max_update_rate` на типе портит **попадание мышью**, а не
только числа. Рисование идёт, рамочное выделение (оно у движка, не у виджета)
идёт, а щелчок не попадает. Под тормозом было семь типов, из них четыре с
интерактивом: `unit_maker_siege_banner` (кнопка, `onclick`,
`alwaystransparent = no`), `unit_marker_compass` (кнопка),
`unit_marker_item_units` (`overlappingitembox`, `alwaystransparent = no`) и
`unit_marker_item_layout` — `vbox`, то есть раскладка, и устаревшая раскладка
самый вероятный виновник промаха.

**0.2.0 сужен до трёх без кнопок и раскладки** — `unit_marker_item_modifiers`,
`unit_marker_item_unit_info`, `unit_marker_battle_side_flank`, — и в них же оба
тяжёлых шаблона (24 и 44 покадровых выражения). **Сломается щелчок и так —
подход по значкам отрядов закрыт**, третьего прогона не просить.

**И до этого та же сборка в лаунчер не попала**: в `metadata.json` не было
...
[`archive/testlog_0918_cm_perf.md`](archive/testlog_0918_cm_perf.md).
