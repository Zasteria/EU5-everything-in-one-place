...
**2026-09-19, `widget_probe` — перепись корня закрыта, и зонд получил то
единственное число, которое двигается.** Скриншот плюс логи (первый раз, когда
они пригодились: жалоба была «зонд ничего не видит», и ответ лежал в
`error.log`).
...
**И зонд писал в лог по три строки на каждую такую строку каждый кадр**: 698
строк в `error.log` и 3425 в `debug.log` за короткую сессию. Всё остальное в
логе чисто — четыре строки `glorpUI_country_header.gui:34`, и больше ничего.
Мёртвые строки из зонда убраны.

**Что вместо них.** Он сказал прямо: «все эти замеры я делать не собираюсь
целенаправленно, для этого твои зонды есть». Поэтому зонд теперь меряет сам:
привод раз в реальную секунду считает тик, месячный такт CMF закрывает счёт, и
первая строка показывает **сколько реальных секунд занял каждый из последних
шести игровых месяцев**. Это ровно та величина, на которую он жалуется, и один
скриншот показывает всю кривую. В игре не прогонялось.
