...
**Ряд жертвы показал «детей в ряду: 2» и слова «жертва» на экране не было.**
Два чтения: кнопка сработала (тогда `PdxGuiDestroyWidget` из мода **работает**),
либо виджет не родился вовсе. Нажимал он её или нет, из скриншота не видно —
**вопрос задан, ответ сюда**.

**И ещё два его наблюдения тем же сообщением.** Играть с открытым зондом
неудобно, окно не сворачивалось — исправлено отдельной кнопкой. И `cm_perf` он
снял: **тормозит по-прежнему, и тем сильнее, чем больше держава и чем больше
точек авторасширения**. Это форма «цена растёт с числом обрабатываемых точек», а
не постоянная цена, снятая воротами 09-18.
