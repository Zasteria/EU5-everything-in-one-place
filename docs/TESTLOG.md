...
**2026-09-14, ванильный костыль автостроя, сборка `fa76e2` — первый прогон.**
CM выключен. Его отчёт целиком, тремя строками, плюс скриншот окна здания
(«Глинокопня в селении Бенешов», `0 (+1) / 8` — то есть стройка первого яруса
там действительно началась).

| что | что вышло | что из этого следует |
| --- | --- | --- |
| «первый ярус» | **работает, но иногда нужно нажать дважды** | строки датамодели появляются не в том кадре, в котором скрипт написал список: один `PdxGuiTriggerAllAnimations` их не застаёт |
| «авторасширение» | **галочка не встаёт ни у одного здания** | ветка галочки будилась `_show`, а он ловит **переход** в видимость — строка, родившаяся уже видимой, молчит. Стройка в том же приводе работала, потому что её строки будит анимация. **Это диф, а не догадка: два механизма в одном файле, работает тот, что с анимацией** |
| иконка отмашки | **всегда горит**, не переключается серая/цветная | у ванильной кнопки просто не было второго значка: подсветку я не сделал вовсе |

**И третья причина, найденная по этому же следу:** ни один из новых
`scripted_gui` не объявлял `saved_scopes`, хотя собственный комментарий этого
мода говорит, что **это и есть способ получить `AddScope`** (`bag_wtp_window_gui.txt:21`),
и CM объявляет его ровно у тех, кто берёт скоупы. Нажатия на локацию и на
провинцию передают `wtp_location`, цена — `wtp_cost`, память о заказанном —
`wtp_building`. Это отдельно объясняет «иногда не срабатывает».

**Сделано по итогам (прогона не видело):** отмашка перестроена на тот механизм,
который в его игре работает, — два прохода анимацией: интерфейс сперва отвечает
про каждое здание, горит ли оно (`MakeScopeBool(IsAutoExpand(...))`) и отдаёт
здание обратно (`Building.MakeScope`), скрипт складывает список того, что надо
нажать, второй проход жмёт его **без условий**. Отсюда же честная подсветка
(`_ax_on`/`_ax_off` — ответ игры, а не память мода) и кнопка о двух смыслах, как
у CM. У стройки добавлен второй круг, а заказанный тип уходит из очереди тем же
нажатием, так что повтор ничего не задваивает.
