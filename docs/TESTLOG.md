...
CM выключен. Сборки `be45c5`, `fa76e2`, `970a21`. **Попытка снята целиком по его
решению, дерево мода откачено**; что из неё стоит знать —
[`archive/wtp_vanilla_autoexpand_attempt.md`](archive/wtp_vanilla_autoexpand_attempt.md).

| что | что вышло |
| --- | --- |
| «первый ярус» | **работает**, стройка начинается (скриншот: «Глинокопня в селении Бенешов», `0 (+1) / 8`) — **но иногда нужно нажать дважды** |
| «авторасширение» | **галочка не встала ни разу**: ни у стоящего здания, ни у строящегося, ни в одной из трёх сборок |
| иконка отмашки | всегда горела, не переключалась — подсветки у ванильной кнопки просто не было |

**Что эти прогоны установили.** `_show` ловит **переход** в видимость, а не
рождение: строка, родившаяся уже видимой, молчит — стройка в том же приводе
работала только потому, что её строки будит `PdxGuiTriggerAllAnimations`. И ни
один новый `scripted_gui` не объявлял `saved_scopes`, то есть не получал
`AddScope` вовсе. **Чего не установили:** доходит ли анимация до строк,
созданных из скриптового списка, и переживает ли `Building` дорогу
скрипт → список → датамодель. Числа, которые бы это разделили, с прогона не
вернулись — третья сборка была уже последней.

**Его слово:** «Просто буду использовать с CM, либо не буду использовать
авторасширение вовсе.»
